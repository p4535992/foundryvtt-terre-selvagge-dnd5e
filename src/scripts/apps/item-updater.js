...
export class ItemUpdater {
  ...
  async _replaceItemOnActor(itemUuid, force = false) {
    const toReplace = await fromUuid(itemUuid);
    const isCompendium = Boolean(this.item.compendium);
    if (itemLinkModuleActive && (isCompendium || this.item.getFlag("item-linking", "isLinked"))) {
      if (force === false) {
        const type = "Item";
        const confirm = await Dialog.confirm({
          title: `${game.i18n.format("DOCUMENT.Delete", { type })}: ${this.item.name}`,
      ...
      await toReplace.update({
        flags: {
          "item-linking": {
            isLinked: true,
            baseItem: isCompendium ? this.item.uuid : this.item.getFlag("item-linking", "baseItem"),
          },
        },
      });
      return true;
    }
    const obj = this.item.toObject();
  ...
  async _generateContent(actor) {
    // const rows = game.actors.reduce((acc, a) => {
    const rows = [actor].reduce((acc, a) => {
    ...
    d.element.find(".replace-button").on("click", async (ev) => {
      ...
      const del = await this._replaceItemOnActor(uuid);
      if (del) el.closest("tr").remove();
      if (d.rendered) d.setPosition({ height: "auto" });
    });
  }

  async itemUpdaterOnActor() {
    const d = new Dialog(
      {
          ...
          fix: {
            ...
            callback: async (html) => {
              ...
              for (const btn of btns) {
                await this._replaceItemOnActor(btn.dataset.uuid, true);
              }
            },
          },
          gucci: {
