...
export class ToolTipHelpers {
  ...
  static wrapElement(parser, note) {
    ...
    const pageType = getProperty(note?.entry?.pages.contents[0], "flags.monks-enhanced-journal.type") ?? "";
  ...
  static defaultParser(note) {
    ...
    if (rollTable && rollTable.results) {
      rollTable.results.forEach((result) => {
        imgElements += `
                    <li>
                        <img src="${result.img}" style="width: 35px; height: 35px; margin-right: 5px;">
                        <span>${result.text}</span>
                    </li>
                `;
      });
    }
    const defaultPermission = note.entry.data.ownership.default;
    ...
    const element = $(`
            <div>
                ...
                <div class="w3-container">
                    ...
                    <ul class="w3-ul w3-margin-top" id="${ulId}">
                        ${imgElements}
                    </ul>
                    <p>Scoperto?<input id="${checkboxId}" type="checkbox" ${
      isChecked ? "checked" : ""
    }></p> <!-- New checkbox -->
                </div>
            </div>
  ...
  static monksEnhancedParsingWrong(note, type) {
    if (type === "quest") {
      ...
      const questStatus = getProperty(page, "flags.monks-enhanced-journal.status") || "Status not available";
      const items = getProperty(page, "flags.monks-enhanced-journal.rewards")?.[0]?.items || [];
      let itemsList = "";
      if (items.length > 0) {
        itemsList += "<ul>";
      ...
      const currency = getProperty(page, "flags.monks-enhanced-journal.rewards")?.[0]?.currency || {};
      let rewards = "";
  ...
  <div>
    ...
    <h3>Stato: ${getProperty(page, "flags.monks-enhanced-journal.alignment")}</h3>
  ${gmOnlyLine}
  ...
  `)[0];
      ...
      const checkboxId = `checkboxEncounter-${ToolTipHelpers.noteCounter}`;
      const element = $(`
              <div>
