...
if (!dualWielderItem) {
  dualWielderFeat = !!(
    actor.items.getName("Dual Wielder") ?? actor.items.find((item) => item.name.includes("Dual Wield"))
  );
        ...
        containerSecondary.addEventListener("click", async function getSecondary(event) {
          ...
          if (target.nodeName == "IMG" && (secondaryWeaponID = target.getAttribute("id"))) {
            ...
            if (dueling) {
              ...
              chosenInitialSecondaryContent = chosenInitialSecondaryContent.replace(
                "Items available",
...
updates = updates.concat(
  actor.items
    .filter((i) => i.name !== weaponMain.name && i.name !== weaponSecondary?.name)
    .filter((i) => i.type === "weapon" || i.getRollData().item.armor?.type === "shield")
    .map((i) => {
...
async function revertWeapons(weapons) {
  ...
  let updateparts = [];
  let weaponsTodeleteIDs = weapons
    .filter((i) => i.flags.world?.originalWeaponLoadout?.originalWeaponId)
    .map((i) => i.id);
  ...
  if (!!weaponsTodeleteIDs.length) {
    if (weaponsTodeleteIDs.length > 1) {
      ...
      if (originalWeaponId1 === originalWeaponId2) {
        ...
        updateparts = [
          { _id: originalWeaponId1, "system.quantity": originalWeapon1quantity + 1 },
          { _id: originalWeaponId2, "system.quantity": originalWeapon2quantity + 1 },
        ];
        await actor.updateEmbeddedDocument("Item", updateparts);
        await actor.deleteEmbeddedDocuments("Item", [weapons[0].id, weapons[1].id]);
      }
    } else if (weaponsTodeleteIDs.length === 1) {
      let originalWeaponId = weapons[0]?.getFlag("world", "originalWeaponLoadout.originalWeaponId")
        ? weapons[0].getFlag("world", "originalWeaponLoadout.originalWeaponId")
        : weapons[1].getFlag("world", "originalWeaponLoadout.originalWeaponId");
  ...
  if (!Array.from(weapons).length) {
    ...
    updateparts = weapons[0].getFlag("world", "originalWeaponLoadout.updateWeapon");
    updateparts = [updateparts].concat(weapons[1].getFlag("world", "originalWeaponLoadout.updateWeapon")).concat([
      { _id: weapons[0].id, "flags.world.-=originalWeaponLoadout": null },
      { _id: weapons[1].id, "flags.world.-=originalWeaponLoadout": null },
    ]);
  }
...
async function changeWeapons(weaponM, weaponS) {
  ...
  //Main weapon
  let {
    item: { quantity: quantityM, properties: propertiesM, range: rangeM, actionType: actionTypeM, damage: damageM },
  } = weaponM.getRollData();
  ...
  if (weaponM) {
    ...
    //Versatile weapons
      ...
      if (!!damageM.versatile) {
        ...
        //to-do check if that is needed and make sure its not!
    ...
    if (weaponS.type === "equipment") {
      ...
      if (!twoWeaponFighter) {
        ...
        updateS = {
          ...
          "system.damage": newDamage,
          "system.activation": newActivationTypeS,
          "system.equipped": true,
        };
      } else {
        updateS = {
...
async function splitWeapon(weaponId, hand) {
  ...
  if (hand !== "both") {
    ...
    if (newWeapon.flags.world)
      foundry.utils.mergeObject(newWeapon.flags.world, { originalWeaponLoadout: { originalWeaponId: origWeapon.id } });
    ...
    returns = [
      newItems[0].id,
      { _id: origWeapon.id, "system.equipped": false, "system.quantity": origWeapon.getRollData().item.quantity - 1 },
    ];
    return returns;
  } else {
    //(hand==="both")
    ...
    if (newWeaponM.flags.world)
      foundry.utils.mergeObject(newWeaponM.flags.world, { originalWeaponLoadout: { originalWeaponId: origWeapon.id } });
    else newWeaponM.flags.world = { originalWeaponLoadout: { originalWeaponId: origWeapon.id } };
    if (newWeaponS.flags.world)
      foundry.utils.mergeObject(newWeaponS.flags.world, { originalWeaponLoadout: { originalWeaponId: origWeapon.id } });
    ...
    returns = [
      newItems[0].id,
      newItems[1].id,
      { _id: origWeapon.id, "system.equipped": false, "system.quantity": origWeapon.getRollData().item.quantity - 2 },
    ];
    return returns;
  }
}
  ...
  if (!damageOptions) {
    ...
    if (mainWeapon.getRollData().item.properties.two) return content;
      ...
      weaponsSecondary = actor.itemTypes.weapon
        .filter((i) => i.getRollData().item.properties.lgt)
        .filter((i) => !i.getRollData().item.properties.two); //if not Dual Wielder only Light Weapons
    } else {
    ...
    if (mainWeapon.getRollData().item.quantity <= 1)
      weaponsSecondary = weaponsSecondary.filter((i) => i.id !== mainWeapon.id);
